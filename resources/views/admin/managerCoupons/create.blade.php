        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.managerCoupon.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.manager-coupons.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('coupon_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="coupon_status" id="coupon_status" required>
                                ...
                                @foreach(App\Models\ManagerCoupon::COUPON_STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('coupon_status', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
